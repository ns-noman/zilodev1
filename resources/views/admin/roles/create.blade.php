                            ...
                            <form action="{{ route('roles.store') }}" method="POST">
                                ...
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="role" placeholder="Role Name" name="role" required>
                                    </div>
                                    <div class="row">
                                        @foreach($menus as $key => $menu)
                                            <div class="col-12 pb-0 mb-0">
                                                <div class="form-group">
                                                    <div class="custom-control custom-switch custom-switch-off-danger custom-switch-on-success main-menu m-0">
                                                        <input {{ $loop->iteration==1? 'checked' : null }} name="menu_id[]" type="checkbox" class="custom-control-input" id="main-menu-{{ $menu->id }}" value="{{ $menu->id }}">
                                                        <label class="custom-control-label" for="main-menu-{{ $menu->id }}">{{ $menu->menu_name }}</label>
                                                        <hr class="m-1 border border-1">
                                                    </div>
                                                    <div class="row ml-2">
                                                        @foreach($menu->submenu as $submenukey => $submenu)
                                                            <div style="width: 20%">
                                                                ...
                                                                <div class="custom-control custom-switch custom-switch-off-warning custom-switch-on-success sub-menu">
                                                                    <input name="menu_id[]" type="checkbox" class="custom-control-input main-menu-{{ $menu->id }}" main-menu-id="{{ $submenu->parent_id }}" id="sub-menu-{{ $submenu->id }}" value="{{ $submenu->id }}">
                                                                    ...
                                                                    @foreach($submenu->subchild as $subchildkey => $subchild)
                                                                        <div class="col-12">
                                                                            <div class="custom-control custom-switch custom-switch-off-gray custom-switch-on-success child-menu">
                                                                                <input name="menu_id[]" type="checkbox" class="custom-control-input sub-menu-{{ $submenu->id }}" sub-menu-id="{{ $subchild->child_id }}"  id="child-menu-{{ $subchild->id }}" value="{{ $subchild->id }}">
